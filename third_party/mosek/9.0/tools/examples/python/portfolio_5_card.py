...
def markowitz_with_card(n, x0, w, gamma, mu, GT, k):
    with mosek.Env() as env:
        with env.Task(0, 0) as task:
            ...
            # Constraints.
            task.appendcons(2 + 4 * n)
            task.putconbound(0, mosek.boundkey.fx, U, U)
            task.putconname(0, "budget")

            task.putconbound(1 + 4 * n, mosek.boundkey.up, -inf, k)
            task.putconname(0, "cardinality")
            ...
            # y variables.
            task.putaijlist(n * [1 + 4 * n], range(offsety, offsety + n), n * [1.0])            
            task.putaijlist(range(1 + 3 * n, 1 + 4 * n),
...
if __name__ == '__main__':
    ...
    for k in range(1, n+1):
        xx = markowitz_with_card(n, x0, w, gamma, mu, GT, k)
        expret = sum([xx[i]*mu[i] for i in range(n)])
        print("Bound {0}:  x = {1}  Return = {2}".format(k, xx, expret))